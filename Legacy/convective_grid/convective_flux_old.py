"""
Convective Flux Solver - Iterative 1D Atmospheric Column Model
...
Based on mixing length theory:
    F_conv = ρ * c_p * alpha^2 * sqrt(g/T) * (N - N_ad)^(3/2)
...
# Ideal gas constant in erg mol^-1 K^-1
...
def calculate_c_p(n_dof: int, mmw: float) -> float:
    ...
    Returns:
        c_p in erg g^-1 K^-1
    """
    # Using R/mmw instead of converting to molecules:
    # c_p = (2 + n_dof) * R / mmw
    # This is equivalent to (2 + n_dof) * k_B / m where m = mmw/N_A
    c_p = (2 + n_dof) * R / mmw
    return c_p
...
def adiabatic_gradient(g: float, c_p: float) -> float:
    ...
    """
    # Work in cgs units for consistency with ergs
    # Convert g from m/s^2 to cm/s^2
    g_cgs = g * 100.0  # cm/s^2
    
    # N_ad = g / c_p
    # Units: (cm/s^2) / (erg g^-1 K^-1)
    # 1 erg = 1 g cm^2 s^-2, so: (cm/s^2) / (g cm^2 s^-2 g^-1 K^-1) = K/cm
    N_ad_cm = g_cgs / c_p  # K/cm
    
    # Convert to K/m: 1 m = 100 cm, so multiply by 100
    N_ad = N_ad_cm * 100.0  # K/m
...
def convective_flux(rho: np.ndarray, c_p: float, alpha: float, g: float,
    ...
    Args:
        rho: Density at layer centers (g/cm^3) - need to interpolate from interfaces
        c_p: Specific heat capacity (erg g^-1 K^-1)
        ...
        N: Temperature gradient at layer centers (K/m)
        N_ad: Adiabatic gradient (K/m)
        mmw: Mean molecular weight (g/mol) - not used in this simplified version
    
    Returns:
        F_conv: Convective flux at layer centers (erg cm^-2 s^-1)
    """
    # Calculate (N - N_ad), but only where N > N_ad
    delta_N = N - N_ad
    delta_N = np.maximum(delta_N, 0.0)  # Floor at zero to avoid complex numbers
    
    # Calculate sqrt(g/T) - need T in layer centers
    # g in m/s^2, T in K, so sqrt(g/T) in (m/s^2 / K)^(1/2)
    sqrt_g_T = np.sqrt(g / T)
    
    # Calculate (N - N_ad)^(3/2)
    delta_N_power = delta_N ** 1.5
    
    # Full formula: F_conv = ρ * c_p * alpha^2 * sqrt(g/T) * (N - N_ad)^(3/2)
    # Using alpha directly (dimensionless)
    F_conv = rho * c_p * alpha**2 * sqrt_g_T * delta_N_power
...
def run(n_layers: int = N_LAYERS, max_z: float = MAX_Z,
    ...
    print(f"Temperature update method: {'Energy conservation (1/(ρc_p))' if use_energy_conservation else 'Constant = 1.0'}")
    print()
    ...
    for step in range(max_steps):
        ...
        if use_energy_conservation:
            ...
            # Simplified version: constant = 1.0 (as per Overview.txt specification)
            DT_CONSTANT = 1.0
            dT = -dt * DT_CONSTANT * dF_dz_erg_cm3_s
        ...
        if step == 0 and debug:
            ...
            if use_energy_conservation:
                ...
                print(f"  constant: {DT_CONSTANT} (simplified, as per Overview.txt)")
            ...
            # Show the constant used (depends on use_energy_conservation)
            if use_energy_conservation:
                ...
                print(f"    dT = -dt * constant * dF_dz = -{dt} * {DT_CONSTANT} * {dF_dz_erg_cm3_s[max_dT_idx]:.2e}")
