...
def solve_implicit_convection(
    ...
    for mask_outer in range(1, icfg.max_mask_outer + 1):
        ...
        for _newton in range(icfg.max_newton):
            ...
            if residual_norm <= icfg.residual_tolerance and step_norm <= icfg.step_tolerance:
                converged_inner = True
                break
            # First iteration: allow residual-only entry when step not yet defined.
            if _newton == 0 and residual_norm <= icfg.residual_tolerance:
                step_norm = 0.0
                converged_inner = True
                break
        ...
        # Conservative projection with ordinary unfrozen closure.
