...
def evaluate_trial(
    ...
    *,
    ...
    """Evaluate F_rad+F_conv for a column state from layer specific enthalpy [J/kg].

    ``enthalpy`` is specific enthalpy on layers (same units as ``thermo.enthalpy(T)``),
    not layer-integrated enthalpy (do not multiply by ``grid.layer_mass``).
    """
